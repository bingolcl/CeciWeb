...
namespace AddressBook
{
	...
	class US_State
	{
		...
		public static US_State FromInt(int input)
		{
			US_State[] states = StateArray.States();

			if (input >= 0 && input < states.Length)
				return states[input];
			else
				throw new IndexOutOfRangeException(String.Format("Invalid state index {0}", input));
		}
	}
